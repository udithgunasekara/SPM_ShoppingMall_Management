...
const ReadGiftcard = ({ giftcards, handleEditGiftcard, handleDeleteGiftcard, setIsGiftcardFormOpen }) => {
  ...
  useEffect(() => {
    if (searchTerm === "") {
      setfilterdata(giftcards); // If no search term, display all data
    } else {
        ...
        <div className="p-6">
            ...
            <div className="bg-white p-6 rounded-lg shadow-md">
                <div className="flex justify-between items-center mb-6 space-x-4">
                    ...
                    />
                    <button onClick={() => setIsGiftcardFormOpen(true)} className="bg-indigo-600 text-white px-6 py-2 rounded-lg hover:bg-indigo-700 transition duration-300">
                        Add New Promotion
                    </button>
                </div>
                        ...
                        <tbody className="bg-white divide-y divide-gray-200">
                            {filterdata.map((item) => (
                                <tr key={item.id} className="hover:bg-gray-50 transition duration-200 ease-in-out">
                                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{item.store}</td>
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{item.price}</td>
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{item.validity.toDate().toLocaleDateString()}</td>                                    
                                    <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium flex items-center space-x-2">
                                        <button onClick={() => handleEditGiftcard(item)} className="text-blue-600 hover:text-blue-800 px-3 py-1 rounded-full text-xs font-semibold bg-blue-100 hover:bg-blue-200 transition duration-300">Edit</button>
                                        <button onClick={() => handleDeleteGiftcard(item.id)} className="text-red-600 hover:text-red-800 px-3 py-1 rounded-full text-xs font-semibold bg-red-100 hover:bg-red-200 transition duration-300">Delete</button>
